...
class LocationRepository(private val context: Context) : FavoriteRepository {
    ...
    suspend fun fetchFavoritesFromApi(): Result<List<FavoriteLocation>> = withContext(Dispatchers.IO) {
        try {
            ...
            if (BuildConfig.DEBUG_LOGS) {
                Log.d(TAG, "API'den favoriler çekiliyor, token: ${token.take(5)}...")
                Log.d(TAG, "Token uzunluğu: ${token.length}")
                Log.d(TAG, "Token formatı kontrol ediliyor...")
            }
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    if (BuildConfig.DEBUG_LOGS) {
                        Log.d(TAG, "API yanıt body: $locationsList")
                    }
                    ...
                    Log.e(TAG, "Hata detayları: ${response.errorBody()?.string()}")
    ...
    suspend fun createFavoriteOnApi(favorite: FavoriteLocation): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
            ...
            if (BuildConfig.DEBUG_LOGS) {
                Log.d(TAG, "Bearer Token kullanılıyor: ${token.take(5)}...")
            }
