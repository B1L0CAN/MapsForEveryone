...
class MainActivity : AppCompatActivity(), OnInitListener {
    ...
    // Çift tıklama için değişkenler
    private var homeClickCount = 0
    private var favoritesClickCount = 0
    private var settingsClickCount = 0
    private var lastClickedItemId = -1
    private val resetClickDelay = 2000L // 2 saniye
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        try {
            ...
            // Navigation Controller'ı ayarla, ancak otomatik yönlendirmeyi kapat
            // Çift tıklama sistemimizi kullanacağız
    ...
    private fun setupViews() {
        // BottomNavigationView'a özel tıklama dinleyicisi ekle
        binding.bottomNavigationView.setOnItemSelectedListener(null) // Önce mevcut listener'ı temizle
        ...
        binding.bottomNavigationView.setOnNavigationItemSelectedListener { item ->
            when (item.itemId) {
                R.id.navigation_home -> {
                    ...
                    if (homeClickCount == 1) {
                        // İlk tıklamada sadece sesli oku
                        speak("Ana Sayfa ekranına geçmek için tekrar tıklayın")
                        
                        // Tıklama sayacını belirli bir süre sonra sıfırla
                        binding.root.postDelayed({
                            if (lastClickedItemId == R.id.navigation_home) {
                                homeClickCount = 0
                            }
                        }, resetClickDelay)
                        
                        return@setOnNavigationItemSelectedListener false // Sayfayı değiştirme
                    } else if (homeClickCount == 2) {
                        // İkinci tıklamada sayfayı değiştir
                ...
                R.id.navigation_favorites -> {
                    ...
                    if (favoritesClickCount == 1) {
                        // İlk tıklamada sadece sesli oku
                        speak("Favoriler ekranına geçmek için tekrar tıklayın")
                        
                        // Tıklama sayacını belirli bir süre sonra sıfırla
                        binding.root.postDelayed({
                            if (lastClickedItemId == R.id.navigation_favorites) {
                                favoritesClickCount = 0
                            }
                        }, resetClickDelay)
                        
                        return@setOnNavigationItemSelectedListener false // Sayfayı değiştirme
                    } else if (favoritesClickCount == 2) {
                        // İkinci tıklamada sayfayı değiştir
                        val transaction = supportFragmentManager.beginTransaction()
                        transaction.setTransition(androidx.fragment.app.FragmentTransaction.TRANSIT_NONE)
                ...
                R.id.navigation_settings -> {
                    ...
                    if (settingsClickCount == 1) {
                        // İlk tıklamada sadece sesli oku
                        speak("Ayarlar ekranına geçmek için tekrar tıklayın")
                        
                        // Tıklama sayacını belirli bir süre sonra sıfırla
                        binding.root.postDelayed({
                            if (lastClickedItemId == R.id.navigation_settings) {
    ...
    fun onUserLoggedIn() {
        try {
            ...
            // Ana sayfa butonunu seçilmiş göster ama sayfaya geçiş yapmasını engelle
